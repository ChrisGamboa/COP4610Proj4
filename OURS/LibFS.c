...
{
  /* YOUR CODE */
  // declare a bitmap that corresponds to one sector
  char bitmap[SECTOR_SIZE];

  // local copy of nbits to manipulate
  int numberOfOnes = nbits;

  // loop through NUM amount of times(num sectors) and write these bitmap buffers to disk
  // while setting appropriate amount of 1s and 0s
...
static int illegal_filename(char* name)
{
  ...
  int i;
  //Checks that each character is legal
  for (i = 0; i < strlen(name); i++) {
    int x = (int) name[i];
    if ( (x >= 48 && x <= 57) || (x >= 65 && x <= 90) || (x >= 97 && x <= 122) || (x >= 45 && x <= 46) || x == 96 ){
